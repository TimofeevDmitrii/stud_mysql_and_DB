...
#ALTER DATABASE lesson4 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci; 
USE lesson4;
...
INSERT INTO users (id, first_name, last_name, e_mail)
VALUES
...
-- Задача 1. Выбрать всех пользователей, указав их id, имя и фамилию, город, аватарку (используя вложенные запросы)

SELECT id, CONCAT(first_name, ' ', last_name) AS 'Пользователь',
	(SELECT hometown FROM `profiles` WHERE users.id = user_id) AS 'Город',
    (SELECT filename FROM media 
		WHERE id = (SELECT photo_id FROM `profiles` WHERE users.id = user_id)
    ) AS 'Аватарка'    
FROM users;
...
SELECT filename
FROM media
WHERE user_id = (SELECT id FROM users WHERE e_mail='arlo50@example.org') 
...
-- не работает, если вместо IN поставить равно
/*
SELECT COUNT(user_id) AS total_likes_female FROM likes 
WHERE media_id = 
(SELECT id FROM media WHERE user_id = 
...
SELECT users.id, users.first_name, users.last_name, COALESCE(SUM(cnt_friends),0) AS cnt
FROM users
LEFT JOIN (
SELECT id, first_name, last_name, COUNT(*) AS cnt_friends
FROM users JOIN 
